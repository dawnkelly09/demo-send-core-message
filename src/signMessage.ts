...
export async function signEvmMessage() {
  console.log(`Attempting to load keystore for account nickname: ${ACCOUNT_NAME}`);
  
  // Create an ethers.js provider
  const provider = new ethers.JsonRpcProvider(RPC_URL);
  
  // Load encrypted keystore
  const keystorePath = path.join(os.homedir(), '.foundry', 'keystores', 'SEPOLIA');
  ...
  console.log(`Signer balance: ${ethers.formatEther(await provider.getBalance(await signer.getAddress()))} native currency`);
